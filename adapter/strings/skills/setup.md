...
3. **Verify activation.** Re-run `spt adapter list`: `claude-spt` must now appear **active** (no
   `deregistered`). Spot-check the profile resolves: `spt adapter get-string claude-spt:ccs adapter_label`
   (the shipped overlay — `:ccs`). Report the active state.
   ...
   live-capable and the command decides ready vs live; reach for `--adapter claude-spt:ccs`
   only to select the ccs override profile.)
